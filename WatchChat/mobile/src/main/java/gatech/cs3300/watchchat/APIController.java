...
public class APIController extends Activity {
    ...
    public void authenticateUser(String username) {
        ...
        client.get(API_ROOT + "/users/auth", params, new AsyncHttpResponseHandler() {
            @Override
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                String response = new String(responseBody);
                System.out.println("login page " +response);
            ...
            public void onFailure(int statusCode, Header[] headers, byte[] responseBody,
                                  Throwable error) {
                System.out.println("API failure handler");
    ...
    public ArrayList<String>  getAllUsers() {
        ...
        AsyncHttpClient client = new AsyncHttpClient();
        client.get(API_ROOT + "users/all", new AsyncHttpResponseHandler() {
            @Override
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                try {
                    ...
                    for (int i = 0; i < jsonArray.length(); i++) {
                        String string = (String) jsonArray.getJSONObject(i).getString("username");
                        userList.add(string);
                    }
                } catch (JSONException e) {
                    System.out.println("JSONException in getAllUsers: " + e.getMessage());
                }
            }
            @Override
            public void onFailure(int statusCode, Header[] headers, byte[] responseBody,
                                  Throwable error) {
                Toast.makeText(getApplicationContext(), "Failure in getting contacts",
                        Toast.LENGTH_LONG).show();
            }
        });
    ...
    public void createUser(String username) {
        ...
        RequestParams params = new RequestParams();
        params.put("username", username);
        client.post(API_ROOT + "users", params, new AsyncHttpResponseHandler() {
            @Override
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                String response = new String(responseBody);
